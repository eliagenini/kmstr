...
--                                last_modified,
--                                ROW_NUMBER()
--                                OVER (PARTITION BY vin, DATE_TRUNC('day', last_modified) ORDER BY last_modified DESC) AS rn
--                         FROM kmstr.mileages)
-- SELECT id,
--        vin,
--        mileage_km,
...
--                                   level,
--                                   last_modified,
--                                   ROW_NUMBER() OVER (PARTITION BY vin ORDER BY last_modified DESC) AS rn
--                            FROM kmstr.fuel_level)
-- SELECT id,
--        vin,
--        level,
...
--         DATE_TRUNC('day', captured_timestamp) AS date,
--         mileage_km,
--         ROW_NUMBER() OVER (PARTITION BY vin, DATE_TRUNC('day', captured_timestamp) ORDER BY captured_timestamp DESC) AS rn
--     FROM
--         kmstr.mileages
-- ), latest_mileage AS (
--     SELECT
